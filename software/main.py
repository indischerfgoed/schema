...
def main():
    ...
    for s, terms in ap.get_terms_per_schema().items():
        for term in sorted(terms):
            if term in ap.mappings and ap.mappings[term]['@id'] in id_to_slug:
                full = ap.mappings[term]['@id']
                term_contents = term_to_markdown(term, full, predecessors, id_to_slug, ap.id_to_term, schemas)
                markdown_to_file(term_contents, id_to_slug[full])
...
def term_to_markdown(term, uri, predecessors, id_to_slug, id_to_term, schemas):
    ...
    for bc in breadcrumbs:
        contents += ' > '.join([get_reference(str(c), id_to_slug, id_to_term) for c in bc]) + '\n'
    contents += '\n'
    ...
    if RDFS.Class in types:
        ...
        if len(direct_properties) > 0:
            ...
            for prop in direct_properties:
                # Property (with link)
                contents += get_reference(str(prop), id_to_slug, id_to_term) + ' | '

                # Range (with links)
                range = [o for o in schemas.objects(prop, RDFS.range)] + [o for o in schemas.objects(prop, URIRef(RANGE_INCLUDES))]
                contents += ', '.join(map(lambda r: get_reference(str(r), id_to_slug, id_to_term), range)) + ' | '
        ...
        if len(range) > 0:
            ...
            for property in range:
                ref = get_reference(str(property), id_to_slug, id_to_term)
                ...
                domain_str = ', '.join(map(lambda d: get_reference(str(d), id_to_slug, id_to_term), domain))
        ...
        if len(more_specific) > 0:
            ...
            for c in more_specific:
                ref = get_reference(str(c), id_to_slug, id_to_term)
    ...
    if RDF.Property in types:
        ...
        if len(domain) > 0:
            ...
            for item in domain:
                contents += ' - ' + \
                    get_reference(str(item), id_to_slug, id_to_term) + '\n'
            contents += '\n\n\n'
